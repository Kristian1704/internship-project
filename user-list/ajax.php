    ...
    if (isset($_POST["action"])&&$_POST['action'] == 'register')
{
    ...
    $location = str_replace("http://localhost/Internship_Project/","",mysqli_real_escape_string($conn, $_POST['oldPath']));
...
elseif (isset($_POST["action"])&&$_POST['action'] == 'list') {
    ...
    if (isset($_POST["email_search"]) && $_POST['email_search']!='')
    {
    ...
    $query_with_ftl = "SELECT COUNT(*) AS allcount 
                       FROM  adm 
                       where  1 = 1 $data_flt_query $emailFilter $phone_flt_query  $searchQuery";
    ...
    $query_user = "SELECT * FROM adm WHERE 1=1 $emailFilter $data_flt_query $phone_flt_query ";
    ...
    foreach ($data as $key => $row) {
        $image = '';
        if ($row['images']) {
            $image = '<img src = "http://localhost/Internship_Project/' . $row['images'] . '" width = "50px" height = "50px" > ';
        }
        ...
        $table_data[] = array("id" => $row['ID'],
            ...
            "action" => "<div class='row mt-2'>
                            <nobr class='mx-4'>
                                ...
                                <button id='userdeleteid' type='button' class='btn btn-primary' onclick='deleteusermodal(" . $row['ID'] . ")' data-toggle='modal' data-target='#deleteModal'>DELETE</button>
                           </nobr>
                         </div> "
...
elseif (isset($_POST["action"])&&$_POST['action'] == 'edit') {
    ...
    exit();
}elseif (isset($_GET["action"])&&$_GET['action'] == 'get_all_numbers'){
    ...
    exit;
}elseif (isset($_GET["action"])&&$_GET['action'] == 'get_all_emails'){
